...
import numpy as np
import requests
from compare import compare_climate_vs_capitalism
...
if "selected_year" not in st.session_state:
    st.session_state.selected_year = int(min(years))

st.sidebar.title("🌍 Filters")
metric = st.sidebar.selectbox("Metric", [
    "Total Population", "Waste Management Score", "Recycling Score",
    "Overall Sustainable Development Goal Score"
])
st.sidebar.title("📍 Navigation")
view = st.sidebar.radio("Go to", ["Global Map", "Compare Metrics"])
if view == "Compare Metrics":
    compare_climate_vs_capitalism(df)
    st.stop()
...
colormap = cm.linear.Blues_09.scale(log_vmin, log_vmax)
...
if st.session_state.selected_country:
    ...
    default_y = st.session_state.get("selected_metric", "Total Population")

    numeric_cols = [
        "Total Population", "Waste Management Score",
        "Recycling Score", "Overall Sustainable Development Goal Score"
    ]

    x_axis = st.selectbox("X-axis", ["year","Property Rights", "Government Integrity", "Judicial Effectiveness",
...
with st.container():
    ...
    with col2:
        new_year = st.slider("Select Year", min_value=int(min(years)), max_value=int(max(years)),
                             value=selected_year, step=1, key="slider_year")
...
colormap.add_to(m)
map_data = st_folium(m, width=950, height=620)
